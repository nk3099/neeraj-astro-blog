...
A vanilla JSON-RPC endpoint will accept calls from *anyone*. That’s cool for demos, but risky in production. We need to lock things down with **authentication**.
...
async function test() {
    ...
    console.log("✅ With Auth - add([5, 3]):", await sendRpcRequest(jsonrpc.request(1, "add", [5, 3]), false, true));
    console.log("✅ With Auth - greet({ name: 'Alice' }):", await sendRpcRequest(jsonrpc.request(2, "greet", { name: "Alice" }), false, true));
    console.log("✅ With Auth - notification:", await sendRpcRequest(jsonrpc.notification("log", {
      message: "Hello"
    }), true, true));
  
    console.log("🚫 No Auth - add([1, 2]):", await sendRpcRequest(jsonrpc.request(3, "add", [1, 2]), false, false));
  }
